...
export const home = async (req, res) => {
  const videos = await Video.find({})
    .sort({ createdAt: "desc" })
    .populate("owner"); //await을 사용하여 js에서 db와 통신하도록 기다려줌. 통신 후에 아래 코드 실행, desc을 통해 생성순서대로 내림차순 정렬.
...
export const watch = async (req, res) => {
  ...
  if (!video) {
    ...
    return res
      .status(404)
      .render("404", { pageTitle: "Error: Video Not Found" });
  }
...
export const getEdit = async (req, res) => {
  ...
  if (String(video.owner) !== String(_id)) {
    //video의 owner id와 현재 로그인 돼있는 user의 id가 다를경우 video edit불가능
...
export const deleteVideo = async (req, res) => {
  ...
  const { _id } = req.session.user; //login돼있는 user의 id
  if (String(video.owner) !== String(_id)) {
    //video의 owner id와 현재 로그인 돼있는 user의 id가 다를경우 video delete 불가능
    req.flash("error", "Not Authorized");
    return res.status(403).redirect("/");
  }
  if (!video) {
...
export const search = async (req, res) => {
  ...
  let videos = [];
  if (keyword) {
    //search input에 무엇인가 써서 search를 했다면.
...
export const registerView = async (req, res) => {
  //어떠한 템플릿을 return하지 않음. post해도 그냥 그 url그대로 유지.
  const { id } = req.params;
  const video = await Video.findById(id);
  if (!video) {
