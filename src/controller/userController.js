...
export const finishGithubLogin = async (req, res) => {
  ...
  if ("access_token" in tokenRequest) {
    ...
    if (!emailObj) {
      //email은 필수 조건이므로. 이메일이 없으면 로그인할수 없음.
...
export const postEdit = async (req, res) => {
  ...
  if (username != user.username) {
    ...
    if (exists) {
      ...
      return res
        .status(400)
        .render("user/edit-profile", { pageTitle: "Edit Profile" });
    }
  }
  if (email != user.email) {
    //email이 변경되었을때
...
export const postChangePw = async (req, res) => {
  ...
  if (!ok) {
    //db에 저장된 pw와 form에 기입한 pw가  다를떄
    req.flash("error", "The current Password is Incorrect");
    return res
      .status(400)
      .render("user/changePw", { pageTitle: "Change Password" });
  }
  if (newPw != newPw1) {
